...
test_that("wflow_start copies files correctly", {
  ...
  site_dir <- tempfile(tmpdir = normalizePath("/tmp"))
...
test_that("wflow_start can handle relative path to current directory: .", {
  ...
  # start project in a tempdir
  site_dir <- tempfile("test-start-", tmpdir = normalizePath("/tmp"))
  dir.create(site_dir)
  cwd <- getwd()
  setwd(site_dir)
  on.exit(setwd(cwd))
...
test_that("wflow_start can handle relative paths to non-existent directories", {
  ...
  tmp_dir <- tempfile("test-start-relative-", tmpdir = normalizePath("/tmp"))
  dir.create(tmp_dir)
  cwd <- getwd()
  setwd(tmp_dir)
  on.exit(setwd(cwd))
...
test_that("wflow_start can handle deeply nested paths that need to be created", {
  ...
  # Create and move to a temp directory
  tmp_dir <- tempfile("test-deeply-nested-", tmpdir = normalizePath("/tmp"))
  dir.create(tmp_dir)
  cwd <- getwd()
  setwd(tmp_dir)
  on.exit(setwd(cwd))
...
test_that("wflow_start can handle deeply nested paths that need to be created and begin with ./", {
  ...
  # Create and move to a temp directory
  tmp_dir <- tempfile("test-deeply-nested-plus-cwd-", tmpdir = normalizePath("/tmp"))
  dir.create(tmp_dir)
  cwd <- getwd()
  setwd(tmp_dir)
  on.exit(setwd(cwd))
...
test_that("wflow_start can handle deeply nested paths that need to be created and use relative paths", {
  ...
  # Create and move to a temp directory
  tmp_dir <- tempfile("test-deeply-nested-plus-relative-", tmpdir = normalizePath("/tmp"))
  dir.create(tmp_dir)
  cwd <- getwd()
  setwd(tmp_dir)
  on.exit(setwd(cwd))
...
test_that("wflow_start throws error when given a deeply nested path that needs to be created, uses relative paths, and is contained within a Git repository", {
  ...
  # Create and move to a temp directory
  tmp_dir <- tempfile("test-deeply-nested-plus-relative-git-", tmpdir = normalizePath("/tmp"))
  dir.create(tmp_dir)
  cwd <- getwd()
  setwd(tmp_dir)
  on.exit(setwd(cwd))
