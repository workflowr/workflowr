...
Next add and commit your R Markdown files using Git at the command-line:

```bash
git add analysis/*Rmd
```

Finally, update your R Markdown files to use the workflowr template (you can
first run `wflow_update` with no arguments to preview the potential changes).
Afterwards, build and commit the website using `wflow_commit`:
...
If you are using version control, first commit your R Markdown files in 
`analysis/` and then run `wflow_update` to convert the R Markdown files to use
the workflowr template:

```{r}
wflow_update(dry_run = FALSE)
```

If you are not using Git, then you'll have to specify the files to update using
the function `wflow_convert`.
