...
directory contains the important configuration files `_site.yml`, which you can
...
To do this, login to your account on GitHub and create a new repository
following these [instructions][new-repo]. The screenshot below shows the menu in
the topright of the webpage.

![Create a new repository on GitHub. © 2018 GitHub Inc. All rights reserved.](https://help.github.com/assets/images/help/repository/repo-create.png)
...
repository. Run the `wflow_git_remote()` command below in the R console, replacing
...
Now you can push your files to GitHub with the function
`wflow_git_push()`^[Unfortunately this can fail for many different reasons. If
you already regularly use `git push` in the Terminal, you will probably want to
continue using this. If you don't have Git installed on your computer and thus
must use `wflow_git_push()`, you can search the [git2r
...
repository would be hosted at the URL `myname.github.io/myproject/`^[It
...
and open it in your preferred text editor (e.g. RStudio). For the purpose of
this vignette, the code below creates a minimal file that only contains a title:
...
workflowr reproducibility report. You can click on any of the bullet points to
learn more about the check, why it's important, and whether or not the file
passed or failed. You'll notice that the first check failed because the R
Markdown file had uncommitted changes. This is OK now since the file is a draft.
Once you are ready to publish it to share with others, you can use
`wflow_publish()` to ensure that any changes to the R Markdown file are
committed to the Git repository prior to generating the results.

In order to make it easier to navigate to your new file, you can include a link
to it on the main index page. First open `analysis/index.Rmd`. Second paste
...
Lastly, push the changes to GitHub with `wflow_git_push()`^[Alternatively you
can run `git push` in the Terminal or use the RStudio Git Pane.] to deploy these
...
1. Open a new or existing R Markdown file in `analysis/`
...
1. Push the changes to GitHub with `wflow_git_push()` (or `git push` in the
Terminal)
