...
.project <- file.path(.tmp, "new-project")
...
workflowr is hosted in a [GitHub repository][repo]. Install the most recent
stable release with devtools.
...
It also changes the working directory to `~/new-project` and initializes a Git
...
the corresponding HTML files in `docs/`. To view the site, run `wflow_view`,
which by default opens the file `docs/index.html` in your browser:
...
```{r wflow-build-dry-run, echo=2}
cat("describe new project\n", file = "analysis/about.Rmd", append = TRUE)
...
can run `wflow_build` again and `wflow_view("first-analysis.Rmd")` (these can be
run from the base directory of your project or any subdirectory).
...
```{r wflow_publish, echo=2:3}
setwd("analysis")
# Assuming working directory is `analysis/`. Run getwd() to confirm
wflow_publish(c("about.Rmd", "index.Rmd", "first-analysis.Rmd"))
setwd("..")
```
...
1. Run `wflow_build` and `wflow_view` to view the results as they will appear on the website (alternatively press the Knit button in RStudio)
...
```{r republish}
wflow_publish(republish = TRUE, dry_run = TRUE)
```
