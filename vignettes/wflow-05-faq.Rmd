...
Another alternative is to upload your workflowr website to [Amazon s3][s3]. s3 is
an object storage service for the Amazon cloud, and can be used to host static
websites. Basic HTTP authentication can be accomplished using [CloudFront][cloudfront],
Amazon's content delivery network, and [Lamba@Edge][lambda], which enables the 
execution of serverless functions to customize content delivered by the CDN.
This [blog post][hackernoon] goes into more detail about what that all means.
A more detailed guide to setting up the bucket is [here][kynatro]. Some
templates for scripting the process are [here][dumrauf].
