---
title: "Hosting worklowr using GitLab"
...
"Getting started" vignette, however we can't use the `wflow_git_remote`
command as it assumes we will be using GitHub. When you get to that stage follow
...
as a URL for the hosting instance.

We now want to connect our local git repository to the one we have just created
on GitLab. Helpully GitLab provides us instructions on how to do that under
"Existing Git respository" on our new repository page. The simplest thing to
do is copy these commands and run them in your local repository directory. If
you can't see the commands or want to do it manually the most important parts
are:

```
git remote add origin HOSTING_URL/myname/myproject.git
...
```
sed s/docs/public/ analysis/_site.yml
```
