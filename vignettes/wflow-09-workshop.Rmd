...
knitr::opts_chunk$set(eval = FALSE, fig.align = "center", out.width = "90%")
```
...
select `workflowr project`.
    ...
    ```{r rstudio-create-project, eval=TRUE, echo=FALSE}
    knitr::include_graphics("img/rstudio-create-project.png")
    ```

    ```{r rstudio-project-type, eval=TRUE, echo=FALSE}
...
your computer, and choose `Create Project`.
    ...
    ```{r rstudio-workflowr-template, eval=TRUE, echo=FALSE}
...
it for editing. Write a short introductory message.
...
created^[Note that the default working directory for a workflowr project is the
root of the project. Hence the relative path is `data/teeth.csv`. The working
directory can be changed via the workflowr option `knit_root_dir` in `_workflowr.yml`.
See `?wflow_html` for more details.].
...
analysis, click on the workflowr button at the top and select the "Checks" tab.
    ...
    ```{r workflowr-report-checks, eval=TRUE, echo=FALSE}
...
first started) but not its corresponding HTML file.
    ...
    ```{r fig-status, eval=TRUE, echo=FALSE}
...
HTML files.
    ...
    ```{r workflowr-past-versions-1, eval=TRUE, echo=FALSE}
...
After it updates, scroll back down and click on the URL.
    ...
    ```{r github-pages-settings, eval=TRUE, echo=FALSE}
...
of the Rmd file on GitHub.
    ...
    ```{r workflowr-past-versions-2, eval=TRUE, echo=FALSE}
...
Following similar steps, you can do the same for one of your own research
...
1. I wasn't able to push to GitHub with `wflow_git_push()`.
    ...
    you can push to Git using another technique, but this will require that you
